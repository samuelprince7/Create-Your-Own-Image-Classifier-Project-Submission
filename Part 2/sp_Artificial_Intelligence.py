...
from ProcessImageNimshow import process_image, imshow

import json

from sp_DataTransformations1 import data_transforms 

###999with open('ImageClassifier/cat_to_name.json', 'r') as f:
###999    cat_to_name = json.load(f)
...
def train_the_model(model, training_dataloaders, val_dataloaders, learn_rate, epochs, device): 
    ...
    #use the GPU if it is avaialbe
          #as the user indicated by not setting --gpu to 'False'
    #we define the device inside of the function as well, just to be sure we are using 'cuda' if GPU is available
    #this will have already been printed out to the command line user via train.py
    #but we want to ensure we have access to the device inside of this function as well
    ###999 device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')      
    #define the loss     
...
###999    torch.save(model, save_origin_location + '/model_checkpoint1.pth')
...
def simple_load_rebuild_model(path):
    ...
    classifier = nn.Sequential(nn.Linear(checkpoint['input_size'], checkpoint['hidden_layer1']),
                               ...
                               nn.ReLU(),
                               nn.Dropout(p=0.5),
                               nn.Linear(checkpoint['hidden_layer2'], 102),
                               nn.LogSoftmax(dim=1))
    model.classifier = classifier 
...
def predict1(image_path, device, rebuilt_detailed_model, topk=5):    
    ...
    ###999 device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # gpu_is_on torch.cuda.is_available() 
...
def sp_sanity_checker(cat_to_name, image_path, device, rebuilt_detailed_model, checkpoint, top_probs):
...
#999    for key, values in dict_origin_class2idx.items():
#999        for k, v in p_c_dict.items():
#999            if str(key) == str(k):
#999                new_order_prob_class_dict[values]=v
