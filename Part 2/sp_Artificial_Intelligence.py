...
def train_the_model(model, training_dataloaders, val_dataloaders, learn_rate, epochs): 
    ...
    #use the GPU if it is avaialbe
          #as the user indicated by not setting it to 'False'
    #we define the device inside of the function as well, just to be sure we are using 'cuda' if GPU is available
    #this will have already been printed out to the command land user via train.py
    #but we want to ensure we have access to the device inside of this function as well
...
def sp_save_model(model, training_datasets, input_size, output_size, epochs, learning_rate, arch, hidden_layer1, hidden_layer2, save_origin_location, save_detailed_location):      
    '''extremely important output of the model captured including
    optimizer_state, class_to_idx, state_dict '''
    #saves the model checkpoint
    torch.save(model, save_origin_location + '/model_checkpoint1.pth')
...
#rebuilt_model = simple_load_rebuild_model('model_checkpoint1.pth')
#rebuilt_detailed_model = simple_load_rebuild_model('detailed_checkpoint1.pth')
       
          
#please note that predict1 takes in ONLY the rebuilt_model
          # and NOT rebuilt_detailed_model
          
def predict1(image_path, rebuilt_model, topk=5):    
    #def predict(image_path, rebuilt_model, gpu_is_on, topk=5):
    # def predict1(image_path, rebuilt_model, gpu_is_on, topk=top_probs):   
    ...
    '''
    #extra safety precaution so that we use GPU if it is available 
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # gpu_is_on torch.cuda.is_available() 
...
def sp_sanity_checker(cat_to_name, image_path, rebuilt_model, rebuilt_detailed_model, top_probs):
    #you may need to include the entire predict funciton above this
    #use the model_checkpoint
    probs, classes = predict1(image_path, rebuilt_model, topk=top_probs)
    ...
    # but you should still see the pandas dataframe seemlessly display the approriate data inside of the terminal
    plt.figure(1)
    plt.subplot(121)
